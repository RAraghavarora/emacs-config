(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(codeium/metadata/api_key "a6705026-aaa8-4693-b087-5f1ed392c83e")
 '(conda-anaconda-home "C:/Users/raghav/miniconda3")
 '(package-selected-packages nil)
 '(safe-local-variable-values
   '((conda-env-current-name . default)
     (projectile-project-run-cmd .
                                 "python C:/Users/raghav/fast-downward-23.06/fast-downward.py --alias seq-sat-fd-autotune-1 domain.pddl prob.pddl")
     (projectile-project-run-cmd . "python")
     (projectile-project-run-cmd .
                                 "fd --alias lama domain.pddl problem.pddl")
     (projectile-project-run-cmd . "bundle exec jekyll serve")
     (python-shell-virtualenv-root .
                                   "C:/Users/raghav/miniconda3/envs/kaggle")
     (conda-project-env-path . "kaggle")
     (python-shell-virtualenv-root .
                                   "C:/Users/raghav/miniconda3/envs/default")
     (conda-project-env-path . "default"))))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 )
